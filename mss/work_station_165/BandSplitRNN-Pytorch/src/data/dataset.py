...
class SourceSeparationDataset(Dataset):
    """
    Dataset class for working with train/validation data from dataset.
    """
    TARGETS: tp.Set[str] = {'violin', 'piano'}
    ...
    def load_file(
        ...
        if self.is_mono:
            y = torch.mean(y, dim=0, keepdim=True) # combine to 1 channel
        return y
