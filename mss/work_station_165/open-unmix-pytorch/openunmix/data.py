...
from typing import Optional, Union, Tuple, List, Any, Callable, Set
...
def load_datasets(
    ...
    if args.dataset == "aligned":
        ...
        parser.add_argument("--nb-train-samples", type=int, default=250)
...
class SourceFolderDataset(UnmixDataset):
    def __init__(
        ...
        root: str,
        split: str = "train",
        target_dir: str = "violin",
        interferer_dirs: List[str] = ["piano"],
        ext: str = ".wav",
        nb_samples: int = 1000,
        seq_duration: Optional[float] = None,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target", type=str, default="violin")
    ...
    newdataset = f"../N250_{args.target}"
    ...
    for k in tqdm.tqdm(range(len(train_dataset))):
        ...
        if args.save:
            # print(x.T.shape, train_dataset.sample_rate)
            sf.write(
                f"{str(output_dir)}/mixture.wav",
                mix.T,
                train_dataset.sample_rate)
            sf.write(
                f"{str(output_dir)}/{args.target}.wav",
                tg.T,
                train_dataset.sample_rate)
            sf.write(
