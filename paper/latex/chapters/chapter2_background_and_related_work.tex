...
\begin{figure}[H]
    ...
    \caption{小提琴與鋼琴演奏G4音高三秒的特徵圖：(a) 鋼琴，(b) 小提琴，(c) 混合}
...
\section{文獻回顧}
\subsection{音源分離相關研究}
音源分離(Music Source Separation, MSS)為MIR中的一個重要領域，日常生活中我們所聽到的音樂大多數是來自多個音源的混合音訊，
許多人可能希望調整混合音訊中的某種樂器的聲音平衡、改變聲音的空間位置，
甚至將混合音訊與每個存在於其中的音樂來源作為其他MIR任務的訓練資料，
因此，若是可以取得每個音樂來源的單獨音訊，上述所說的應用都能夠實現。

而音源分離是盲源分離(Blind Source Separation, BSS)的一個子任務，更早出現在MIR的領域中，
盲源分離不只用於處理音訊資料，在生物醫學訊號與通信訊號的處理上也會使用到此技術。
主要方法有兩種，獨立成分分析(Independent Components Analysis, ICA)~\cite{comon1994independent}，
...
另外也常用主成分分析法(Principal Component Analysis, PCA)~\cite{mackiewicz1993principal}
來降低資料為度與消除雜訊。
然而盲源分離的方法對於分離複雜度高的訊號，表現並不理想。

近年來隨著深度學習領域的蓬勃發展，大家漸漸將音訊資料的盲源分離變成一個子任務，也就是音源分離，
目前最常見的目標為分離一首包含人聲、貝斯、鼓與其他(非前三者的所有音訊)的歌曲，
...
大部分會使用全連接神經網路(Fully-Connected Neural Network, FC)、
...
目標為小提琴時在SDR指標上約提升了1.28DB，鋼琴則提升了4.46DB。
