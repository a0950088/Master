...
\fig[1][fig:fig-ch3-system-structure-overview-fixed][!hbt]{ch3/fig-system-structure-overview-fixed.png}
[系統架構圖][系統架構圖]

\cref{fig:fig-ch3-system-structure-overview-fixed} (A)
為本系統的整體架構，此系統可分為離線處理與線上處理兩個階段，
離線處理階段會將混合音源分離成小提琴參考音訊與鋼琴參考音訊作為線上處理階段的資料，
線上處理階段接收現場音訊，並結合參考音訊來計算目前現場音訊對應到的參考音訊位置並輸出估計伴奏音訊。
請注意本研究將小提琴與鋼琴之混合音訊作為主要資料，
並專注在當演奏者樂器為小提琴或鋼琴的情況下來使用本系統。

\cref{fig:fig-ch3-system-structure-overview-fixed} (B)
為音源分離模組的細部架構，
採用深度學習模型將小提琴與鋼琴的混合音訊分離為小提琴參考音訊與鋼琴參考音訊。
此模組包含了小提琴音源分離模型與鋼琴音源分離模型，
另外在訓練階段，本研究設計了頻帶切割估計方法提升模型的分離效果。
關於模型架構、頻帶切割估計方法會在\ref{ch3-st-mss-module}節做討論。

\cref{fig:fig-ch3-system-structure-overview-fixed} (C)
為音樂追蹤模組的細部架構，
我們將音源分離模組分離出來的參考音訊作為音樂追蹤模組的資訊，
此模組將會即時接收現場音訊並使用小提琴參考音訊來對齊現場音訊，進而獲得目前現場音訊演奏的位置，
由於小提琴參考音訊與鋼琴參考音訊為完美對齊的，
我們可以透過目前對齊到的小提琴參考音訊位置取得目前鋼琴參考音訊的位置做為估計伴奏音訊輸出，達成即時追蹤與伴奏的效果。
此模組使用Python 3.9開發，
...
資料管理者管理所有音訊的資料與特徵、
音樂偵測器即時偵測音樂是否開始、
粗略位置估計器估計整段小提琴參考音訊中目前現場可能的演奏位置，提供決策決定者更多選擇、
...
BSRNN的輸入為頻譜圖$X\in \mathbb{C} ^{F\times T}$，如\cref{fig:fig-ch3-BSRNN-input-feature}所示。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{ch3/fig-BSRNN-input-feature.png}
    \caption{BSRNN 輸入頻譜圖}
    \label{fig:fig-ch3-BSRNN-input-feature}
\end{figure}
...
接著會根據自定義的頻帶切割點$G = (G_1, G_2, \dots G_i), \sum_{i = 1}^{K} G_i = F $
...
在BSRNN的論文中，他們將模型應用於分離人聲、貝斯、鼓與其他這四種目標音源，
我們將訓練目標改為分離小提琴與鋼琴這兩種音源，來達成本研究的目的。
...
首先我們對每種樂器選取約15分鐘的的音訊資料，
對於小提琴資料，我們分別選取有不同速度與不同難度的技巧(例如：雙音、跳弓等)表現的音訊，
而鋼琴資料則是選取有不同速度的音訊，這些資料並不會加入模型的訓練。
...
此圖為兩種樂器中其中一段音訊資料的波型圖(上)與經過FFT轉換後的$F_{band}$圖(下)，
可以看到經過FFT轉換後我們可以清楚的看到每個頻率點在這段音訊中佔有的成分。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{ch3/fig-bandwidth-estimate.png}
    \caption{兩種資料的波型圖與經過FFT轉換後的頻譜圖：(a) 小提琴波型圖、(b) 鋼琴波型圖、(c) 小提琴FFT頻譜圖、(d) 鋼琴FFT頻譜圖}
...
計算X與Y所有時間點的距離，如\cref{fig:fig-ch3-dtw-cell-cost-matrix}所示。
\begin{equation}
...
\begin{figure}[!hbt]
    \centering
    \includegraphics[width=0.5\linewidth]{ch3/fig-dtw-cell-cost-matrix.png}
    \caption{DTW單格成本距離矩陣示意圖}
    \label{fig:fig-ch3-dtw-cell-cost-matrix}
\end{figure}
...
在啟動追蹤系統前，資料管理者會拿到音源分離模組分離出來的小提琴參考音訊與鋼琴參考音訊，
...
為了讓用來對齊現場音訊的小提琴參考音訊特徵更接近人類的聽覺系統，
我們對小提琴參考音訊的STFT頻譜圖做梅爾頻譜分析，得到高解析度特徵。
...
追蹤系統啟動後，現場音訊以單聲道、樣本數為Window size的大小傳送到系統，
對於現場音訊的特徵處理方法與上述相同，
但由於在接收音訊時並不像非串流音訊一樣直接拿到整段音訊，
因此必須確保接收到足夠的樣本數才能擷取特徵，否則可能導致特徵蒐集不完全而失真。
因此當系統啟動後，我們將現場音訊記錄下來，當音訊累積足夠的長度時再擷取高/低解析度特徵。

\cref{fig:fig-ch3-feature}為現場音訊與小提琴參考音訊之特徵圖，
可以看到高解析度特徵的時間軸切片較細，低解析度特徵時間軸切片較粗。
另外現場音訊雖然是以Window size大小處理特徵，
但當追蹤結束時，現場音訊特徵與使用整段音訊做特徵擷取的小提琴參考音訊特徵，
在視覺化上並沒有特別明顯的差異(無特徵失真的現象)。

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{ch3/fig-feature.png}
    \caption{現場與參考的高低解析度特徵圖：(a)現場高解析度，(b)現場低解析度，(c)參考高解析度，(d)參考低解析度}
...
我們將現場音訊擷取為高解析度特徵並與小提琴參考音訊開頭0.5秒的高解析度特徵
計算DTW對齊成本${DTW_{cost}}$。
...
粗略位置估計器負責使用低解析度特徵計算
目前最新現場音訊幀與小提琴參考音訊的相似度，
並挑出相似度高的前8個粗略估計位置提供給決策決定者參考。

當新的現場音訊幀經過特徵擷取後，
粗略位置估計器會獲得新的現場低解析度特徵$Live_{low}[t] \in \mathbb{R}^{1 \times F}$，
$t$為最新的低解析度特徵時間點。
此時粗略位置估計器會計算$Live_{low}[t]$與整個小提琴參考低解析度特徵$Ref_{low} \in \mathbb{R}^{N \times F}$
...
負責即時計算現場音訊的高解析度特徵與小提琴參考音訊的高解析度特徵之即時對齊位置，
