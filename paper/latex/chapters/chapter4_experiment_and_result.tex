...
接著我們使用上面訓練出效果最好的BSRNN模型來分離幾種音訊，分別為測試資料集中其中一首音訊、
貝多芬第五號小提琴奏鳴曲第一樂章與韋瓦第四季-夏的第三樂章。並展示分離結果頻譜圖。

首先\cref{fig:fig-mss-test-dataset-results}為測試資料集中的其中一首音訊，
我們展示輸入(混合音訊)、鋼琴分離結果、鋼琴Ground truth、小提琴分離結果與小提琴Ground truth的頻譜圖，
可以看到第二張鋼琴分離結果與第三張鋼琴Ground truth頻譜圖幾乎一模一樣，
而在小提琴分離結果與Ground truth的比較中，可以看到基頻與泛音位置幾乎沒有偏差，
另外在基頻以下，Ground truth的頻譜圖有一些錄音時會有的雜訊，但我們的分離結果中並沒有將這些雜訊也一起分離出來，
我認為是因為模型並沒有針對雜訊方面特別訓練，因此剛好將雜訊濾除。
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{ch4/fig-mss-test-dataset-results.png}
    \caption{測試資料集 TheKitchenettes\_Alive\_p0\_v0 結果比較圖：(a)混合音訊、(b)鋼琴音訊、(c)鋼琴Ground truth、(d)小提琴音訊、(e)小提琴Ground truth}
    \label{fig:fig-mss-test-dataset-results}
\end{figure}

\cref{fig:fig-mss-beethoven-results}為貝多芬第五號小提琴奏鳴曲第一樂章的結果圖，
由於沒有Ground truth可以比較，我們只展示輸入與分離結果，這首曲子小提琴與鋼琴較沒有太多同時重疊的旋律，
且旋律組成較簡單，因此可以看到分離結果都很好的將鋼琴與小提琴各自的部分分離出來。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{ch4/fig-mss-beethoven-results.png}
    \caption{貝多芬第五號小提琴奏鳴曲第一樂章結果圖：(a)混合音訊、(b)鋼琴音訊、(c)小提琴音訊}
    \label{fig:fig-mss-beethoven-results}
\end{figure}

\cref{fig:fig-mss-vivaldi-summer-3rd-results}為韋瓦第四季-夏第三樂章的結果圖，
與上一首貝多芬相比，這首曲子較多小提琴與鋼琴同時重疊的旋律，且速度較快，旋律組成也較複雜，
因此可以看到雖然分離結果大部分都有將兩種樂器的旋律分離出來，但在相同重疊處(開頭處)可能會因為頻率成份多寡不好預測，
而導致分離結果音訊聽起來可能會有一點混合音訊的聲音。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{ch4/fig-mss-vivaldi-summer-3rd-results.png}
    \caption{韋瓦第四季-夏\ 第三樂章結果圖：(a)混合音訊、(b)鋼琴音訊、(c)小提琴音訊}
    \label{fig:fig-mss-vivaldi-summer-3rd-results}
\end{figure}

上述所提到的三首曲子之分離結果可由此連結~\cite{YuJieLin2024DifferentMelodyMusicSourceSeparation}查看聆聽。
...
\subsection{音樂追蹤模組評估方法} \label{ch4-subst-music-tracking-module-assessment-method}
我們參考~\cite{Lin2020AHumanComputerDuetSystem}中的評估方法來檢測小提琴演奏追蹤系統的表現，
在~\cite{Lin2020AHumanComputerDuetSystem}中，
評估樂曲為貝多芬的Violin Sonata No.5, Op.24(Spring), movement I. Allegro 前25小節，
他們的現場演奏音訊為鋼琴，並且事先準備好鋼琴參考MIDI音訊與小提琴參考MIDI音訊做為音樂追蹤的參考音訊，
透過即時音樂追蹤系統輸出的對齊位置即時合成估計的小提琴伴奏MIDI音訊。
為了評估系統在不同現場演奏速度下的反應，他們使用四種不同的速度範圍來測試系統，分別為演奏正常速度(115-145bpm)、
慢速(90-120bpm)、快速(135-175bpm)、漸快(由80bpm加速至160bpm)，參考音訊的速度設定為120bpm。
他們錄製現場音訊與估計伴奏音訊，
並使用節拍追蹤工具得到現場音訊與參考音訊之間的節拍位置，進一步生成正確伴奏音訊(Ground truth)，
最後透過DTW計算估計伴奏音訊與正確伴奏音訊的對齊路徑，
使用\cref{eq:latency-function}與\cref{eq:avarage-latency-function}計算瞬間延遲與平均延遲來評估。

本研究的現場音訊為小提琴，估計的伴奏音訊為鋼琴，在現場音訊與參考音訊速度的設定上我們跟隨
...
我們將現場音訊的部分改為使用隨機生成速度的MIDI檔案來評估，
由於MIDI檔案擁有音符開始時間、節拍等等資訊，因此能精準地計算延遲，此外，
我們可以透過現場MIDI音訊生成正確的鋼琴伴奏音訊，因此不需要額外的節拍追蹤工具輔助。
雖然現場音訊為預先設定好的檔案，但這並不影響即時的效果，現場音訊還是會以串流的方式進行處理。
...
由於正確的鋼琴伴奏音訊已經透過MIDI檔案生成，
我們只需要用正確伴奏音訊$Acc_{gt}$與即時音樂追蹤系統估計的伴奏音訊$Acc_{out}$計算DTW對齊路徑，
由於伴奏音訊的時間長度是一致的，
...
$| \Delta t\vert $為估計伴奏音訊的延遲。
\begin{equation}
    \label{eq:avarage-latency-function}
    \frac{1}{T} \sum _{t=0}^{T} | \Delta [t] \vert 
\end{equation}

\subsection{音樂追蹤模組評估結果} \label{ch4-subst-music-tracking-module-assessment-results}
我們顯示系統在四種速度變化下隨時間變化的延遲圖，
也就是估計伴奏音訊比正確伴奏音訊要提前或落後多少個16分音符。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{ch4/fig-slow-latency.png}
    \caption{慢(90-120bpm) 平均延遲：55.04毫秒}
    \label{fig:fig-slow-latency}
\end{figure}

\cref{fig:fig-normal-latency}是一般速度下的延遲圖，由於一般速度的範圍是最接近參考音訊的速度，
因此可以看到整體平均延遲為四種速度下最低的，系統延遲線幾乎保持在y軸為0的地方。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{ch4/fig-normal-latency.png}
    \caption{一般(115-145bpm) 平均延遲：35.81毫秒}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{ch4/fig-fast-latency.png}
    \caption{快(135-175bpm) 平均延遲：62.96毫秒}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{ch4/fig-accelerate-latency.png}
    \caption{加速(80$ \rightarrow $160bpm) 平均延遲：58.73毫秒}
    \label{fig:fig-accelerate-latency}
\end{figure}

另外可以發現在第五小節(m5)與第七小節(m7)時，四種速度的16分音符延遲數皆為負數，也就是比起正確的時間點還要提前，
可能是因為剛好那個時間點為休止符，也就是沒有音樂，因此造成系統先對齊到了最後面的靜音時間點並等待，
導致延遲。

我們也顯示了四種速度的對齊路徑結果圖，橫軸為現場音訊的時間點，縱軸為小提琴參考音訊的時間點，
...
我們使用與\ref{ch4-subst-music-tracking-module-assessment-method}節相同的曲子，
在網路上找到由kopikostar演奏的小提琴與鋼琴合奏版本\cite{kopikostar2024Beethoven}，
並對此混合音訊做音源分離，將分離結果作為音樂追蹤模組的參考音訊。
...
由於沒有正確的伴奏音訊(Ground truth)，
因此我們無法使用\ref{ch4-subst-music-tracking-module-assessment-method}節的方法來評估
與正確伴奏音訊的延遲。
因此我們改由計算系統輸出路徑與DTW離線對齊路徑的平均延遲幀數來評估系統在線上追蹤的表現是否與離線追蹤相當，
這邊的平均延遲幀數代表的是對於線上追蹤表現，是否提前或落後離線追蹤表現。

\subsection{系統評估結果}
下圖為五種不同速度的對齊路徑結果圖，橫軸為現場音訊的時間點，縱軸為小提琴參考音訊的時間點，
(a)圖為追蹤系統在執行時所輸出的結果圖，
(b)圖為離線對齊路徑與線上對齊路徑的比較圖與平均延遲幀數。

\cref{fig:fig-ch4-mss-110bpm-tracking-results}為速度110bpm下的追蹤結果，
由\cref{fig:mss-110bpm-tracking-output}的xy軸可以看出現場音訊的速度比小提琴參考音訊要來的慢，
...
\begin{figure}[H]
    ...
    {\includegraphics[width=0.6\linewidth]{ch4/fig-mss-110bpm-feature.png}}
    ~
    \subcaptionbox
    {系統追蹤輸出展示圖
    \label{fig:mss-110bpm-tracking-output}}
    {\includegraphics[width=0.45\linewidth]{ch4/fig-mss-110bpm-tracking-system-output.png}}
    ~
    \subcaptionbox
    {系統與DTW離線追蹤比較圖
    \label{fig:mss-110bpm-comparision-output}}
    {\includegraphics[width=0.45\linewidth]{ch4/fig-mss-110bpm-offline-and-online-output.png}}
...
由\cref{fig:mss-120bpm-tracking-output}的xy軸可以看出現場音訊的速度也是比小提琴參考音訊的速度稍慢，
...
\begin{figure}[H]
    ...
    {\includegraphics[width=0.6\linewidth]{ch4/fig-mss-120bpm-feature.png}}
    ~
    \subcaptionbox
    {系統追蹤輸出展示圖
    \label{fig:mss-120bpm-tracking-output}}
    {\includegraphics[width=0.45\linewidth]{ch4/fig-mss-120bpm-tracking-system-output.png}}
    ~
    \subcaptionbox
    {系統與DTW離線追蹤比較圖
    \label{fig:mss-120bpm-comparision-output}}
    {\includegraphics[width=0.45\linewidth]{ch4/fig-mss-120bpm-offline-and-online-output.png}}
...
\begin{figure}[H]
    ...
    {\includegraphics[width=0.6\linewidth]{ch4/fig-mss-130bpm-feature.png}}
    ~
    \subcaptionbox
    {系統追蹤輸出展示圖
    \label{fig:mss-130bpm-tracking-output}}
    {\includegraphics[width=0.45\linewidth]{ch4/fig-mss-130bpm-tracking-system-output.png}}
    ~
    \subcaptionbox
    {系統與DTW離線追蹤比較圖
    \label{fig:mss-130bpm-comparision-output}}
    {\includegraphics[width=0.45\linewidth]{ch4/fig-mss-130bpm-offline-and-online-output.png}}
...
由\cref{fig:mss-140bpm-tracking-output}的xy軸可以看出現場音訊比小提琴參考音訊的速度稍快一些，
...
\begin{figure}[H]
    ...
    {\includegraphics[width=0.6\linewidth]{ch4/fig-mss-140bpm-feature.png}}
    ~
    \subcaptionbox
    {系統追蹤輸出展示圖
    \label{fig:mss-140bpm-tracking-output}}
    {\includegraphics[width=0.45\linewidth]{ch4/fig-mss-140bpm-tracking-system-output.png}}
    ~
    \subcaptionbox
    {系統與DTW離線追蹤比較圖
    \label{fig:mss-140bpm-comparision-output}}
    {\includegraphics[width=0.45\linewidth]{ch4/fig-mss-140bpm-offline-and-online-output.png}}
...
由\cref{fig:mss-free-tracking-output}的xy軸可以看出現場音訊比小提琴參考音訊的速度稍慢一些，
...
\begin{figure}[H]
    ...
    {\includegraphics[width=0.6\linewidth]{ch4/fig-mss-free-feature.png}}
    ~
    \subcaptionbox
    {系統追蹤輸出展示圖
    \label{fig:mss-free-tracking-output}}
    {\includegraphics[width=0.45\linewidth]{ch4/fig-mss-free-tracking-system-output.png}}
    ~
    \subcaptionbox
    {系統與DTW離線追蹤比較圖
    \label{fig:mss-free-comparision-output}}
    {\includegraphics[width=0.45\linewidth]{ch4/fig-mss-free-offline-and-online-output.png}}
...
因此追蹤結果圖大多數看起來比\ref{ch4-subst-music-tracking-module-assessment-results}節的結果要差。
...
首先我們先錄製了與伴奏音訊完整對齊的音訊作為混合音訊，並將此混合音訊透過音源分離模型分離成參考音訊，
並使用這些音訊進行音樂追蹤。
接著現場演奏我們一樣採自由速度，
...
\begin{figure}[H]
    ...
    {\includegraphics[width=0.6\linewidth]{ch4/fig-free-ours-feature.png}}
    ~
    \subcaptionbox
    {系統追蹤輸出展示圖
    \label{fig:free-ours-ref-tracking-output}}
    {\includegraphics[width=0.4\linewidth]{ch4/fig-free-ours-ref-tracking-system-output.png}}
    ~
    \subcaptionbox
    {系統與DTW離線追蹤比較圖
    \label{fig:free-ours-ref-comparision-output}}
    {\includegraphics[width=0.4\linewidth]{ch4/fig-free-ours-ref-offline-and-online-output.png}}
